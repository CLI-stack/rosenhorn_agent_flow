...
foreach rd (`echo $eff_rd`)
    ...
    if (-e gold.config) then
        source cmds/Env.csh
        RunClock cmds/$target.csh
        echo "$rd,$target running" >> $source_dir/data/${tag}_spec
    else
        echo "$rd,gold.config not exist" >> $source_dir/data/${tag}_spec
    endif
end
