...
if (-e $target.error.log) then
    ...
    if (-e $log_center/$target.reported && $team != "debugger") then
        ...
        set mailErrorReport = `python3 $source_dir/script/read_csv.py --csv $source_dir/assignment.csv | grep "mailErrorReport,"  | awk -F "," '{print $2}'`
        set n_mailErrorReport = `echo $mailErrorReport | wc -w` 
        if ($n_mailErrorReport == 1) then
            if ($mailErrorReport == 1) then
