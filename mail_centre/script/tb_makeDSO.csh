...
if ($n_refDir > 0) then
    foreach rd (`echo $refDir | sed 's/:/ /g' | sed 's/refDir//g'`)
        ...
        if (-e $rd/tile.params) then
            ...
            echo "# tb_make at $rd"
            TileBuilderTerm -x "TileBuilderMakeDSO;touch TileBuilderMakeDSO_$tag.started"
        else
...
foreach t (`echo $tile | sed 's/^tile//g' | sed 's/:/ /g'`)
    ...
    foreach rd (`cat $runDir`)
        ...
        if ($t == $curr_tile) then
            ...
            touch TileBuilderMakeDSO_$tag.started
            rm TileBuilderMakeDSO_$tag.started
            if (-e logs/TileBuilderMakeDSO.log || -e logs/TileBuilderMakeDSO.log.gz) then
                rm logs/TileBuilderMakeDSO.log*
            endif
            echo "# tb_make at $rd"
            TileBuilderTerm -x "TileBuilderMakeDSO;touch TileBuilderMakeDSO_$tag.started"
            set eff_rd = "$eff_rd $rd"
...
foreach rd (`echo $eff_rd`)
    ...
    set n_wait = 0
    while(1)
        if (-e TileBuilderMakeDSO_$tag.started) then
            echo "$t,$rd,TileBuilderMakeDSO passed" >> $source_dir/data/${tag}_spec
            break
        endif
        set n_wait = `expr $n_wait + 1`
        if ($n_wait > 3600) then
            echo "$t,$rd,TileBuilderMakeDSO failed in 60 mins" >> $source_dir/data/${tag}_spec
            break
        endif
        sleep 1
    end
