...
#list#
    The run progress report has been stop.
EOF
echo "$tag | $run_status | ${target_run_dir} | $tasksModelFile"
python $source_dir/py/stopTask.py --tag $tag --status $run_status --reply "replied" --target_run_dir "${target_run_dir}" --source_run_dir "$source_dir" --html $source_dir/data/${tag}_spec.html  --tasksModelFile $tasksModelFile --ins "report_runProgress.csh"
