...
#list#
    The analog has been added to $file :
#table#
tile,runDir,analog
EOF
...
foreach rd (`echo $eff_rd`)
    ...
    if ($n_file == 1)  then
        ...
        foreach ip (`echo $analogip | sed 's/:/ /g' | sed 's/analogip//g'`) 
            echo $ip >> $file
        end
        set analog_list = `echo $analogip | sed 's/:/;/g' | sed 's/analogip//g'`
        echo "$t,$rd,$analog_list" >> $source_dir/data/${tag}_spec
    endif
end
