...
if (-e $source_dir/data/$tag.table && $n_table > 0) then
    python3 $source_dir/script/extract_table_sdc.py --table $source_dir/data/$tag.table --tag $tag
endif

cat >> $source_dir/data/${tag}_spec << EOF
#list#
    The sdc has been hacked:
#table#
tile,runDir,Status
EOF
set eff_rd = ""
...
foreach rd (`echo $eff_rd`)
    ...
    echo "# hack sdc in $rd"
    set n_csh = `ls $source_dir/data/$tag/hack_sdc_*.csh | wc -l`
    if ($n_csh > 0) then
        foreach csh (`ls $source_dir/data/$tag/hack_sdc_*.csh`)
            echo "# run $csh"
            source $csh
        endif
    endif
    echo "# hack sdc in $rd"
    if ($n_file > 0) then
        foreach sdc (`echo $file | sed 's/:/ /g' | sed 's/file//g'`)
            echo "# hack $sdc"
            cat $source_dir/data/$tag/$tag.tcl >> $sdc
            echo "$t,$source_dir/data/$tag/$tag.tcl,$rd/$sdc" >> $source_dir/data/${tag}_spec           
        end
    endif
end
