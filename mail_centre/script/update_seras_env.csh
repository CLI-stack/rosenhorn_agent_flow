...
#text#
    The seras env has been generated:
#table#
EOF
...
foreach t (`echo $tile | sed 's/^tile//g' | sed 's/:/ /g'`)
     if ($n_refDir > 0) then
        foreach baseDir (`echo $refDir | sed 's/:/ /g' | sed 's/refDir//g'`)
            cd $baseDir
            touch TileBuilderUpdateSeras_$tag.started
            rm TileBuilderUpdateSeras_$tag.started
            TileBuilderTerm -x "TileBuilderUpdateSeras;touch TileBuilderUpdateSeras_$tag.started"
...
foreach rd (`echo $eff_rd`)
    ...
    set n_wait = 0
    while(1)
        if (-e TileBuilderUpdateSeras_$tag.started) then
            echo "$t,$rd,TileBuilderUpdateSeras passed" >> $source_dir/data/${tag}_spec
            break
        endif
        set n_wait = `expr $n_wait + 1`
        if ($n_wait > 3600) then
            echo "$t,$rd,TileBuilderUpdateSeras failed in 60 mins" >> $source_dir/data/${tag}_spec
            break
        endif
        sleep 1
    end
