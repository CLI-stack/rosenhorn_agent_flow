...
foreach rd (`echo $eff_rd`)
    ...
    TileBuilderTerm -x "serascmd -find_jobs 'name=~Pt.*Route dir=~$curr_dir' --action stop;"
    set rerun_flag = 0
    TileBuilderTerm -x "serascmd -find_jobs 'name=~Pt.*Route dir=~$curr_dir' --action unskip;"
    TileBuilderTerm -x "serascmd -find_jobs 'name=~Pt.*Route dir=~$curr_dir' --action reset"
    set cmd = ""
...
foreach rd (`echo $eff_rd`)
    cd $rd
    set curr_dir = `pwd | sed 's/\// /g' | awk '{print $NF}'`
    TileBuilderTerm -x "serascmd -find_jobs 'name=~Pt.*Route dir=~$curr_dir' --action run;touch rerun_$tag.started"
