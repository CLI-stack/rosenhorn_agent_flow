...
if ($n_tileStatus > 0) then
    deactivate
        foreach lk (` cat tileStatus.list | sort -k 1 | awk '{print $2}' `)
            cd $lk 
            set run_tcl = "" 
            foreach pt_cmd (`echo $file`)
                set n_pt_cmd = `echo $pt_cmd | grep "Pt.*cmd" | wc -w`
                set n_run_tcl = `cat $source_dir/data/$tag/$tag.tcl | wc -w`
                if ($n_pt_cmd > 0) then
                    if (-e $pt_cmd) then
                    else
                        echo "$pt_cmd not exist in $lk" >> $source_dir/data/${tag}_spec
                    endif
                    if ($n_run_tcl > 0) then
                        TileBuilderTerm -x "TileBuilderLoadTiming --nosi $pt_cmd -a $source_dir/data/$tag/$tag.tcl --nogui"
                    else
                        TileBuilderTerm -x "TileBuilderLoadTiming --nosi $pt_cmd"
                        echo "$n_run_tcl not run tcl"
                    endif
                endif
            end
            echo "$lk" >> $source_dir/data/${tag}_spec
            echo "# open $pt_cmd for $lk"
        end
