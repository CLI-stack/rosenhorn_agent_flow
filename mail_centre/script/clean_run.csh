...
if ($n_refDir == 0) then
    echo "Run dir is not exist or spell wrongly." >> $source_dir/data/${tag}_spec
endif
set tile_filter = ""
...
foreach rd (`echo $eff_rd`)
    ...
    set curr_dir = `pwd | sed 's/\// /g' | awk '{print $NF}'`
    TileBuilderTerm -x "cd ../;TileBuilderClean --destroy $curr_dir"
    echo "$t,$rd,cleaning" >> $source_dir/data/${tag}_spec
end
