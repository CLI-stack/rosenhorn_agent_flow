...
if ($n_file == 0) then
    echo "#text"
    echo "File name not specified." >> $source_dir/data/${tag}_spec
    exit
endif
...
foreach rd (`echo $eff_rd`)
    ...
    foreach f (`echo $file | sed 's/:/ /g' | sed 's/file//g' `)
        set n_placerams = `echo $f | grep placerams | wc -w`
        # write FxFpPlaceMacros.placerams.tcl
        if ($n_placerams > 0) then
            if (-e data/$t.FxFpPlaceMacros.placerams.tcl) then
                rm data/$t.FxFpPlaceMacros.placerams.tcl
            endif
            echo "source /tool/aticad/1.0/src/zoo/PD_agent/tile/project/at2gmd/saveRam_ICC2.tcl" >> data/write_placeram.tcl 
            echo "sh cp ${t}.tcl data/$t.FxFpPlaceMacros.placerams.tcl -f" >> data/write_placeram.tcl
            echo "exit" >> data/write_placeram.tcl
            TileBuilderTerm -x "TileBuilderIntFX data/FpPlaceMacros.nlib -a data/write_placeram.tcl --nogui"
        endif
    end
end
...
foreach rd (`echo $eff_rd`)
    ...
    set t = `grep TILES_TO_RUN $rd/tile.params | grep -v "#" | awk '{print $3}'`
    if ($n_placerams > 0) then
        source $source_dir/script/wait_file_finish.csh data/$t.FxFpPlaceMacros.placerams.tcl
        echo "$t,$rd,data/$t.FxFpPlaceMacros.placerams.tcl generated." >> $source_dir/data/${tag}_spec
    endif
end
