...
set task = "fix_baseDrc"
set task_file = ${task}_$tag
foreach rd (`echo $eff_rd`)
    cd $rd
    set t = `grep TILES_TO_RUN $rd/tile.params | grep -v "#" | awk '{print $3}'`
    TileBuilderTerm -x "TileBuilderIntFX data/FpPlaceMacros.nlib -a /tool/aticad/1.0/src/zoo/PD_agent/tile/fix_baseDrc.tcl --nogui;touch $task_file"
    echo "$t,$rd,$rd/data/fix_baseDrc.def" >> $source_dir/data/${tag}_spec
    chmod 744 tune/FxFpPlaceMacros/FxFpPlaceMacros.postplace.tcl
    echo "read_def data/fix_baseDrc.def" >> tune/FxFpPlaceMacros/FxFpPlaceMacros.postplace.tcl
end
...
foreach rd (`echo $eff_rd`)
    ...
    echo "$t,$rd,FxFpPlaceMacros rerun" >> $source_dir/data/${tag}_spec
end
