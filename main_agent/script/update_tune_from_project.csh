...
#list#
    The tune has been updated from project tune:
#table#
EOF
...
foreach rd (`echo $eff_rd`)
    ...
    if (-e $source_dir/script/project/$project/tune) then
        if ($n_target > 0) then
            echo "# start copy $source_dir/script/project/$project/tune/$target to $rd"
            cp -rf $source_dir/script/project/$project/tune/$target tune/
            echo "$t,$rd,$source_dir/script/project/$project/tune/$target copied done" >> $source_dir/data/${tag}_spec
        else
            echo "# start copy $source_dir/script/project/$project/tune to $rd"
            cp -rf $source_dir/script/project/$project/tune/* tune/
            echo "$t,$rd,$source_dir/script/project/$project/tune copied done" >> $source_dir/data/${tag}_spec
        endif
    endif
end
