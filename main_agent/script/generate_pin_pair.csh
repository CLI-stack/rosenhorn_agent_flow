...
set task = "gen_pin_pair_list"
set task_file = ${task}_$tag
foreach rd (`echo $eff_rd`)
    cd $rd
    set t = `grep TILES_TO_RUN $rd/tile.params | grep -v "#" | awk '{print $3}'`
    TileBuilderTerm -x "TileBuilderIntFX data/FpPlaceMacros.nlib -a /tool/aticad/1.0/src/zoo/PD_agent/tile/optimizer/gen_pin_pair_list.tcl --nogui;touch $task_file"
    echo "$t,$rd,$rd/genVdciPinList.rpt" >> $source_dir/data/${tag}_spec
end
