...
foreach rd (`echo $eff_rd`)
    ...
    if (-e $source_dir/data/$tag.params) then
        set n_description = `grep DESCRIPTION tile.params | wc -w`
        if ($n_description > 0)  then
            set description = `grep DESCRIPTION $source_dir/data/$tag.params | sed 's/DESCRIPTION//g' | sed 's/=//g'`
            set description_orig = `grep "^DESCRIPTION" $rd/tile.params | grep -v "#"`
            echo "$description_orig ; $description" >> $rd/override.params
            TileBuilderTerm -x "TileBuilderGenParams > TileBuilderGenParams.log;touch $task_file"
        else
            grep DESCRIPTION $source_dir/data/$tag.params >> $rd/override.params
            TileBuilderTerm -x "TileBuilderGenParams > TileBuilderGenParams.log;touch $task_file"
        endif
    endif
end
...
foreach rd (`echo $eff_rd`)
    ...
    echo "$t,$rd,add description passed" >> $source_dir/data/${tag}_spec
end
