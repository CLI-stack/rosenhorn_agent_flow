...
if ($n_file == 0) then
    echo "Please specify the pt cmds to run report power, e.g. cmds/PtTimFuncTT0p65vReRouteTyprc100cTT0P65V0CStp.cmd"
    set run_status = "failed"
    exit
endif
cat >> $source_dir/data/${tag}_spec << EOF
#list#
    The power has been reported:
#table#
EOF

echo "tile,runDir,power,rpt" >> $source_dir/data/${tag}_spec
...
foreach rd (`echo $eff_rd`)
    ...
    foreach f (`echo $file | sed 's/:/ /g' | sed 's/file//g' `)
        ...
        echo "source /tools/aticad/1.0/src/zoo/PD_agent/tile/report_power.tcl > rpts/report_power.rpt" > report_power.tcl
        echo "echo finished > rpts/${task}_$tag" >> report_power.tcl
        echo "exit" >>  report_power.tcl
        if ($n_pt_cmd > 0) then
            TileBuilderTerm -x "TileBuilderLoadTiming --nosi $f -a report_power.tcl;touch $task_file"
        else
...
foreach rd (`echo $eff_rd`)
    ...
    set power = `grep "Total Power" rpts/report_power.rpt | awk '{print $4}'`
    echo "$t,$rd,$power,$rd/rpts/report_power.rpt" >> $source_dir/data/${tag}_spec
end
