...
if ($n_regu > 0) then
    echo $regu | sed 's/regu://g' | sed "s/:/|/g" | awk '{print "egrep ","\""$1"\"","tbs.log"}' > $source_dir/data/${tag}/gen_target.csh
    sed -i 's/__/\.\*/g' $source_dir/data/${tag}/gen_target.csh
endif
...
foreach rd (`echo $eff_rd`)
    ...
    if ($n_regu > 0) then
        set target_orig = `source $source_dir/data/${tag}/gen_target.csh | awk '{print $2}'`
    endif

    set rerun_flag = 0
    set cmd = ""
    foreach tg (`echo $target_orig | sed 's/:/ /g' | sed 's/target//g' `)
        set target_list = "$target_list;$tg"
    ...
    set stop_cmd = "serascmd --action stop --jobs $ids"
...
foreach rd (`echo $eff_rd`)
    ...
    while(1)
        if (-e rerun_target_$tag.started) then
            ...
            echo "$t,$rd,$target_list" >> $source_dir/data/${tag}_spec
            set reply = "Success"
            set run_used = 1
