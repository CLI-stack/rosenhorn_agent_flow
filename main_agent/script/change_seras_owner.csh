...
#list#
    The owner of seras has been changed to $vto .
#table#
tile,runDir,status
EOF
...
foreach rd (`echo $eff_rd`)
    cd $rd
    echo "$t,$rd,seras owner change" >> $source_dir/data/${tag}_spec
    touch serasOwner_$tag.started
    rm serasOwner_$tag.started
    TileBuilderTerm -x "seras -disconnect;seras -start;touch serasOwner_$tag.started"
    touch $tag.task 
end
...
foreach rd (`echo $eff_rd`)
    ...
    set n_wait = 0
    while(1)
        if (-e serasOwner_$tag.started) then
            break
        endif
