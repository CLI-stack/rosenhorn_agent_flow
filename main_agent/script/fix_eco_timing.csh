...
foreach rd (`echo $eff_rd`)
    ...
    foreach f (`echo $file | sed 's/:/ /g' | sed 's/file//g' `)
        ...
        set clk = `echo $clk | sed 's/://g' | sed 's/clk//g'`
        echo "source /tools/aticad/1.0/src/zoo/PD_agent/tile/fix_eco_timing.tcl" > fix_eco_timing.$clk.tcl
        if ($n_clk > 0) then
            echo "report_timing -start_end_pair -max_paths 10000  -from $clk -to $clk -group $clk > rpts/post_eco_timing.$clk.rpt" >> fix_eco_timing.$clk.tcl
            echo "source /tools/aticad/1.0/src/zoo/PD_agent/tile/check_timing_margin.tcl" >> fix_eco_timing.$clk.tcl
            echo "check_timing_margin $clk" >> fix_eco_timing.$clk.tcl
        else 
            echo "report_timing  -start_end_pair -max_paths 10000 > rpts/post_eco_timing.$clk.rpt" >> fix_eco_timing.$clk.tcl
        endif
        echo "save_session rpts/post_eco_timing.$clk" >> fix_eco_timing.$clk.tcl
        echo "echo finished > rpts/${task}_$tag" >> fix_eco_timing.$clk.tcl
        echo "exit" >>  fix_eco_timing.$clk.tcl
        if ($n_pt_cmd > 0) then
            TileBuilderTerm -x "TileBuilderLoadTiming --nosi $f -a fix_eco_timing.$clk.tcl;touch $task_file"
        else
...
foreach rd (`echo $eff_rd`)
    ...
    touch $tag.task
    set t = `grep TILES_TO_RUN $rd/tile.params | grep -v "#" | awk '{print $3}'`
    source $source_dir/script/wait_file_finish.csh rpts/${task}_$tag 5000 
    if ($n_clk > 0) then
        echo "$t,$rd,$rd/rpts/post_eco_timing.$clk.rpt;$rd/rpts/check_timing_margin.$clk.rpt" >> $source_dir/data/${tag}_spec
    else
        echo "$t,$rd,$rd/rpts/post_eco_timing.$clk.rpt" >> $source_dir/data/${tag}_spec
    endif
end
